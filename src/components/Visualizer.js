...
const WEIGHTING_MODES = [
  {label: '无', description: '没有衰减'},
  {label: 'A-加权', description: '将低频和高频暗化，以近似人类听觉的敏感度'}
];
const FFT_SIZES = [
      ...
      <div className='Visualizer'>
        <h3 className='Visualizer-toggle'>
          ...
          <label className='inline'><input onClick={this.handleToggleVisualizer}
                        ...
                        name='visualizer-enabled'/>开启</label>
          <label className='inline'><input onClick={this.handleToggleVisualizer}
                        type='radio'
                        value={false}
                        defaultChecked={this.state.enabled === false}
                        name='visualizer-enabled'/>关闭</label>
        </h3>
